...
int main(int argc, char** argv)
{
	...
	/************************************* MTK BOOTIMG ************************************/
	if(kernel[0] == 0x88 && kernel[1] == 0x16 && kernel[2] == 0x88 && kernel[3] == 0x58) { /* 判断是否存在MTK文件头       */
		mtk_file = 1;                                                                      /* 判断是否创建MTK FILE        */
		fseek(f,-header.kernel_size + MTK_MAGIC,SEEK_CUR);                                 /* 指针指向kernel文件头        */
		header.kernel_size -= MTK_MAGIC;                                                   /* 跳过MTK文件头后的kernel大小 */
		total_read += MTK_MAGIC;                                                           /* 读取文件总大小              */
		free(kernel);                                                                      /* 释放kernel占用的内存        */
		kernel = (byte*)malloc(header.kernel_size);                                        /* 分配内存指针                */
		fread(kernel, header.kernel_size, 1, f);                                           /* 读取文件流到内存            */
	}
	/**************************************************************************************/
	...
	if(mtk_file == 1) header.kernel_size += MTK_MAGIC;                                     /* 使扇区大小正确补全          */
	...
	/**************************************************************************************/
	if(ramdisk[0] == 0x88 && ramdisk[1] == 0x16 && ramdisk[2] == 0x88 && ramdisk[3] == 0x58) { /* 判断是否存在MTK文件头        */
		mtk_file = 1;																		   /* 判断是否创建MTK FILE         */
		fseek(f,-header.ramdisk_size + MTK_MAGIC,SEEK_CUR);									   /* 指针指向ramdisk文件头        */
		header.ramdisk_size -= MTK_MAGIC;													   /* 跳过MTK文件头后的ramdisk大小 */
		total_read += MTK_MAGIC;															   /* 读取文件总大小               */
		free(ramdisk);																		   /* 释放ramdisk占用的内存        */
		ramdisk = (byte*)malloc(header.ramdisk_size);										   /* 分配内存指针                 */
		fread(ramdisk, header.ramdisk_size, 1, f);											   /* 读取文件流到内存             */
	}
	/**************************************************************************************/
    FILE *r = fopen(tmp, "wb");
	total_read += header.ramdisk_size;
	...
	if(mtk_file == 1) header.ramdisk_size += MTK_MAGIC;                                   	   /* 使扇区大小正确补全           */
	...
	if(mtk_file != 0) {                                                                    /* M */
		sprintf(tmp, "%s/%s", directory, basename(filename));                              /* T */
		strcat(tmp, "-mtk");                                                               /* K */
		FILE *m = fopen(tmp, "wb");                                                        /* F */
		fwrite("Hewoyi", 6, 1, m);                                                         /* I */
		fclose(m);                                                                         /* L */
	}                                                                                      /* E */
